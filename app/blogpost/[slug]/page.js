...
export async function generateStaticParams() {
  const blogDir = path.join(process.cwd(), "content"); // Absolute path
...
export default async function Page({ params }) {
  const blogDir = path.join(process.cwd(), "content"); // Absolute path to content
  const filePath = path.join(blogDir, `${params.slug}.md`);

  if (!fs.existsSync(filePath)) {
    notFound(); // Show 404 if file does not exist
    return;
  }
